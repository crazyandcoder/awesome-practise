...
import com.crazyandcoder.android.lib.common.widget.recycler.animation.CrazyAlphaInAnimation;
import com.crazyandcoder.android.lib.common.widget.recycler.animation.ICrazyBaseAnimation;
import com.crazyandcoder.android.lib.common.widget.recycler.diff.AbsCrazyBaseAdapterListUpdateCallback;
import com.crazyandcoder.android.lib.common.widget.recycler.diff.AbsCrazyBaseDiffCallback;
import com.crazyandcoder.android.lib.common.widget.recycler.entity.ICrazyExpandable;
import com.crazyandcoder.android.lib.common.widget.recycler.loadmore.AbsCrazyLoadMoreView;
import com.crazyandcoder.android.lib.common.widget.recycler.loadmore.CrazyLoadMoreView;
 ...
 * @CreateDate: 2020/6/18 6:08 AM
 * @UpdateUser: 更新者
 * @UpdateDate: 2020/6/18 6:08 AM
 * @UpdateRemark: 更新说明
 * @Version: 1.0
 */
public abstract class AbsCrazyBaseAdapter<T, K extends CrazyBaseViewHolder> extends RecyclerView.Adapter<K> {

    //load more
    private boolean mNextLoadEnable = false;
    private boolean mLoadMoreEnable = false;
    private boolean mLoading = false;
    private AbsCrazyLoadMoreView mAbsCrazyLoadMoreView = new CrazyLoadMoreView();
    ...
    private ICrazyBaseAnimation mCustomAnimation;
    private ICrazyBaseAnimation mSelectAnimation = new CrazyAlphaInAnimation();
    //header footer
    ...
    protected static final String TAG = AbsCrazyBaseAdapter.class.getSimpleName();
    ...
    /**
     * 该方法有问题，如果配合NestScrollView使用的话，会一直调用加载更多方法,如果单独使用的话不存在该问题
     * 建议直接复写NestScrollView的滚动监听，滑动到底部的时候再加载更多
     *
     ...
     */
    public void setAbsCrazyLoadMoreView(AbsCrazyLoadMoreView loadingView) {
        this.mAbsCrazyLoadMoreView = loadingView;
    }
     ...
     * @return 0 or 1
     */
    public int getAbsCrazyLoadMoreViewCount() {
        if (mRequestLoadMoreListener == null || !mLoadMoreEnable) {
            return 0;
        }
        if (!mNextLoadEnable && mAbsCrazyLoadMoreView.isLoadEndMoreGone()) {
            return 0;
        }
        if (mData.size() == 0) {
            return 0;
     ...
     * @return
     */
    public int getAbsCrazyLoadMoreViewPosition() {
    ...
    public void loadMoreEnd(boolean gone) {
        if (getAbsCrazyLoadMoreViewCount() == 0) {
            return;
        }
        mLoading = false;
        mNextLoadEnable = false;
        mAbsCrazyLoadMoreView.setLoadMoreEndGone(gone);
        if (gone) {
            notifyItemRemoved(getAbsCrazyLoadMoreViewPosition());
        } else {
            mAbsCrazyLoadMoreView.setLoadMoreStatus(AbsCrazyLoadMoreView.STATUS_END);
            notifyItemChanged(getAbsCrazyLoadMoreViewPosition());
        }
    }
    ...
    public void loadMoreComplete() {
        ...
        mLoading = false;
        mNextLoadEnable = true;
        mAbsCrazyLoadMoreView.setLoadMoreStatus(AbsCrazyLoadMoreView.STATUS_DEFAULT);
        notifyItemChanged(getAbsCrazyLoadMoreViewPosition());
    }
    ...
    public void loadMoreFail() {
        ...
        mLoading = false;
        mAbsCrazyLoadMoreView.setLoadMoreStatus(AbsCrazyLoadMoreView.STATUS_FAIL);
        notifyItemChanged(getAbsCrazyLoadMoreViewPosition());
    }
    ...
    public void setEnableLoadMore(boolean enable) {
        int oldLoadMoreCount = getAbsCrazyLoadMoreViewCount();
        mLoadMoreEnable = enable;
        int newLoadMoreCount = getAbsCrazyLoadMoreViewCount();
        ...
        if (oldLoadMoreCount == 1) {
            ...
            if (newLoadMoreCount == 1) {
                mAbsCrazyLoadMoreView.setLoadMoreStatus(AbsCrazyLoadMoreView.STATUS_DEFAULT);
                notifyItemInserted(getAbsCrazyLoadMoreViewPosition());
            }
        }
    }
     ...
     */
    public AbsCrazyBaseAdapter(@LayoutRes int layoutResId, @Nullable List<T> data) {
    ...
    public AbsCrazyBaseAdapter(@Nullable List<T> data) {
        this(0, data);
    }

    public AbsCrazyBaseAdapter(@LayoutRes int layoutResId) {
     ...
     *
     * @param baseQuickDiffCallback implementation {@link AbsCrazyBaseDiffCallback}
     */
    public void setNewDiffData(@NonNull AbsCrazyBaseDiffCallback<T> baseQuickDiffCallback) {
     ...
     */
    public void setNewDiffData(@NonNull AbsCrazyBaseDiffCallback<T> baseQuickDiffCallback, boolean detectMoves) {
        ...
        diffResult.dispatchUpdatesTo(new AbsCrazyBaseAdapterListUpdateCallback(this));
    ...
    public void setNewDiffData(@NonNull DiffUtil.DiffResult diffResult, @NonNull List<T> newData) {
        ...
        diffResult.dispatchUpdatesTo(new AbsCrazyBaseAdapterListUpdateCallback(AbsCrazyBaseAdapter.this));
        mData = newData;
    }
    ...
    /**
     * compatible getAbsCrazyLoadMoreViewCount and getEmptyViewCount may change
     *
    ...
    public int getItemCount() {
        ...
        if (1 == getEmptyViewCount()) {
            ...
            count = getHeaderLayoutCount() + mData.size() + getFooterLayoutCount() + getAbsCrazyLoadMoreViewCount();
        }
        return count;
    }
    ...
    public K onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        switch (viewType) {
            ...
            case HEADER_VIEW:
                ...
                baseViewHolder = createCrazyBaseViewHolder(mHeaderLayout);
                break;
            case EMPTY_VIEW:
                ...
                baseViewHolder = createCrazyBaseViewHolder(mEmptyLayout);
                break;
            case FOOTER_VIEW:
                ...
                baseViewHolder = createCrazyBaseViewHolder(mFooterLayout);
                break;
            default:
    ...
    private K getLoadingView(ViewGroup parent) {
        View view = getItemView(mAbsCrazyLoadMoreView.getLayoutId(), parent);
        K holder = createCrazyBaseViewHolder(view);
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mAbsCrazyLoadMoreView.getLoadMoreStatus() == AbsCrazyLoadMoreView.STATUS_FAIL) {
                    notifyLoadMoreToLoading();
                }
                if (mEnableLoadMoreEndClick && mAbsCrazyLoadMoreView.getLoadMoreStatus() == AbsCrazyLoadMoreView.STATUS_END) {
                    notifyLoadMoreToLoading();
                }
            }
        });
    ...
    public void notifyLoadMoreToLoading() {
        if (mAbsCrazyLoadMoreView.getLoadMoreStatus() == AbsCrazyLoadMoreView.STATUS_LOADING) {
            return;
        }
        mAbsCrazyLoadMoreView.setLoadMoreStatus(AbsCrazyLoadMoreView.STATUS_DEFAULT);
        notifyItemChanged(getAbsCrazyLoadMoreViewPosition());
    ...
    public void onBindViewHolder(@NonNull K holder, int position) {
        ...
        //Do not move position, need to change before AbsCrazyLoadMoreView binding
        autoLoadMore(position);
        int viewType = holder.getItemViewType();

        switch (viewType) {
            case LOADING_VIEW:
                mAbsCrazyLoadMoreView.convert(holder);
                break;
            case HEADER_VIEW:
                break;
            case EMPTY_VIEW:
    ...
    public void setOnItemClick(View v, int position) {
        getOnItemClickListener().onItemClick(AbsCrazyBaseAdapter.this, v, position);
    }
    ...
    public boolean setOnItemLongClick(View v, int position) {
        return getOnItemLongClickListener().onItemLongClick(AbsCrazyBaseAdapter.this, v, position);
    }

    protected K onCreateDefViewHolder(ViewGroup parent, int viewType) {
        return createCrazyBaseViewHolder(parent, mLayoutResId);
    }

    protected K createCrazyBaseViewHolder(ViewGroup parent, int layoutResId) {
        return createCrazyBaseViewHolder(getItemView(layoutResId, parent));
    }

    /**
     * if you want to use subclass of CrazyBaseViewHolder in the adapter,
    ...
    protected K createCrazyBaseViewHolder(View view) {
        Class temp = getClass();
        Class z = null;
        ...
        // 泛型擦除会导致z为null
        if (z == null) {
            k = (K) new CrazyBaseViewHolder(view);
        } else {
            k = createGenericKInstance(z, view);
        }
        return k != null ? k : (K) new CrazyBaseViewHolder(view);
    }
            ...
            if (type instanceof ParameterizedType) {
                ...
                for (Type temp : types) {
                    if (temp instanceof Class) {
                        ...
                        if (CrazyBaseViewHolder.class.isAssignableFrom(tempClass)) {
                            return tempClass;
                        }
                    } else if (temp instanceof ParameterizedType) {
                        Type rawType = ((ParameterizedType) temp).getRawType();
                        if (rawType instanceof Class && CrazyBaseViewHolder.class.isAssignableFrom((Class<?>) rawType)) {
    ...
    /**
     * When the current adapter is empty, the AbsCrazyBaseAdapter can display a special view
    ...
    private void autoLoadMore(int position) {
        ...
        if (mAbsCrazyLoadMoreView.getLoadMoreStatus() != AbsCrazyLoadMoreView.STATUS_DEFAULT) {
            return;
        }
        mAbsCrazyLoadMoreView.setLoadMoreStatus(AbsCrazyLoadMoreView.STATUS_LOADING);
        if (!mLoading) {
            mLoading = true;
            if (getRecyclerView() != null) {
    ...
    private void addAnimation(RecyclerView.ViewHolder holder) {
        if (mOpenAnimationEnable) {
            if (!mFirstOnlyEnable || holder.getLayoutPosition() > mLastPosition) {
                ICrazyBaseAnimation animation = null;
    ...
    public void openLoadAnimation(@AnimationType int animationType) {
        ...
        switch (animationType) {
            case ALPHAIN:
                mSelectAnimation = new CrazyAlphaInAnimation();
                break;
            default:
                break;
        }
     ...
     */
    public void openLoadAnimation(ICrazyBaseAnimation animation) {
        this.mOpenAnimationEnable = true;
     ...
     * then {@link AbsCrazyBaseAdapter#convert(CrazyBaseViewHolder, Object)} will not execute, and will
    ...
    public View getViewByPosition(RecyclerView recyclerView, int position, @IdRes int viewId) {
        ...
        CrazyBaseViewHolder viewHolder = (CrazyBaseViewHolder) recyclerView.findViewHolderForLayoutPosition(position);
        if (viewHolder == null) {
            return null;
        }
    ...
    private int recursiveExpand(int position, @NonNull List list) {
        ...
        for (int i = list.size() - 1; i >= 0; i--, pos--) {
            if (list.get(i) instanceof ICrazyExpandable) {
                ICrazyExpandable item = (ICrazyExpandable) list.get(i);
    ...
    public int expand(@IntRange(from = 0) int position, boolean animate, boolean shouldNotify) {
        ...
        ICrazyExpandable expandable = getExpandableItem(adapterPos);
        if (expandable == null) {
            return 0;
        }
...
//            subItemCount += list.size();
    ...
    private int recursiveCollapse(@IntRange(from = 0) int position) {
        ...
        ICrazyExpandable expandable = (ICrazyExpandable) item;
        if (!expandable.isExpanded()) {
            return 0;
        }
        ...
        for (int i = position + 1, n = mData.size(); i < n; i++) {
            ...
            if (itemTemp instanceof ICrazyExpandable && ((ICrazyExpandable) itemTemp).getLevel() <= itemLevel) {
                break;
            }
    ...
    public boolean hasSubItems(ICrazyExpandable item) {
        if (item == null) {
            return false;
        }
    ...
    public boolean isExpandable(T item) {
        return item instanceof ICrazyExpandable;
    }

    private ICrazyExpandable getExpandableItem(int position) {
        T item = getItem(position);
        if (isExpandable(item)) {
            return (ICrazyExpandable) item;
        } else {
            return null;
        }
    }
    ...
    /**
     * Get the parent item position of the ICrazyExpandable item
     *
     * @return return the closest parent item position of the ICrazyExpandable.
     * if the ICrazyExpandable item's level is 0, return itself position.
    ...
    public int getParentPosition(@NonNull T item) {
        ...
        // if the item is ICrazyExpandable, return a closest ICrazyExpandable item position whose level smaller than this.
        // if it is not, return the closest ICrazyExpandable item position whose level is not negative
        int level;
        if (item instanceof ICrazyExpandable) {
            level = ((ICrazyExpandable) item).getLevel();
        } else {
        ...
        for (int i = position; i >= 0; i--) {
            T temp = mData.get(i);
            if (temp instanceof ICrazyExpandable) {
                ICrazyExpandable expandable = (ICrazyExpandable) temp;
    ...
    public interface OnItemChildClickListener {
         ...
         */
        void onItemChildClick(AbsCrazyBaseAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemChildLongClickListener {
         ...
         * click and held
         *
         * @param adapter  this AbsCrazyBaseAdapter adapter
         * @param view     The childView whihin the itemView that was clicked and held.
         * @param position The position of the view int the adapter
         * @return true if the callback consumed the long click ,false otherwise
         */
        boolean onItemChildLongClick(AbsCrazyBaseAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemLongClickListener {
         ...
         * @return true if the callback consumed the long click ,false otherwise
         */
        boolean onItemLongClick(AbsCrazyBaseAdapter adapter, View view, int position);
    }
    ...
    public interface OnItemClickListener {
         ...
         */
        void onItemClick(AbsCrazyBaseAdapter adapter, View view, int position);
    }
