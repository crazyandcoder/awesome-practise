...
def insert_mysql(id):
    ...
    url = "https://api.eol.cn/gkcx/api/?access_token=&keyword=&page=" + str(
        id) + "&province_id=&request_type=1&school_type=&signsafe=&size=20&special_id=&type=&uri=apidata/api/gk/schoolSpecial/lists"
    ...
    for i in range(length):
        ...
        try:
            ...
            result.append(infoes['data']['item'][i]['is_important'])
            result.append(infoes['data']['item'][i]['level1'])
            result.append(infoes['data']['item'][i]['name'])
            result.append(infoes['data']['item'][i]['school_id'])
            result.append(infoes['data']['item'][i]['school_nature'])
            result.append(infoes['data']['item'][i]['school_sp_id'])
            result.append(infoes['data']['item'][i]['school_type'])
            result.append(infoes['data']['item'][i]['special_id'])
            result.append(infoes['data']['item'][i]['spname'])
            result.append(infoes['data']['item'][i]['type_name'])
            sql = "insert into top_university_school_profession_to_school_info(dual_class_name,f211,f985,gkschoolspecial_id,is_important," \
                  "level1,name,school_id,school_nature,school_sp_id," \
                  "school_type,special_id,spname,type_name )values(%s,%s,%s,%s,%s," \
                  "%s,%s,%s,%s,%s," \
                  "%s,%s,%s,%s)"
...
def main():
    start = time.time()
    pool = Pool(3)
    origin_num = [x for x in range(1, 5045)]
    try:
