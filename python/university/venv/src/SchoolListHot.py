...
def insert_mysql(id):
    ...
    cursor = conn.cursor()
    url = "https://api.eol.cn/gkcx/api/?access_token=&keyword=&page=" + str(
        id) + "&province_id=&school_type=&signsafe=&size=20&sort=view_total&sorttype=desc&type=&uri=apidata/api/gk/school/lists"
    ...
    for i in range(length):
        ...
        try:
            ...
            result.append(infoes['data']['item'][i]['admissions'])
            result.append(infoes['data']['item'][i]['belong'])
            result.append(infoes['data']['item'][i]['central'])
            ...
            result.append(infoes['data']['item'][i]['county_id'])
            result.append(infoes['data']['item'][i]['county_name'])
            result.append(infoes['data']['item'][i]['department'])
            result.append(infoes['data']['item'][i]['doublehigh'])
            ...
            result.append(infoes['data']['item'][i]['is_recruitment'])
            ...
            result.append(infoes['data']['item'][i]['view_month'])
            result.append(infoes['data']['item'][i]['view_total'])
            result.append(infoes['data']['item'][i]['view_total_number'])
            result.append(infoes['data']['item'][i]['view_week'])
            sql = "insert into top_university_school_list_hot(address,admissions,belong,central,city_id," \
                  "city_name,code_enroll,colleges_level,county_id,county_name," \
                  "department,doublehigh,dual_class,dual_class_name,f211," \
                  "f985,is_recruitment,is_top,level,level_name," \
                  "name,nature,nature_name,province_id,province_name," \
                  "school_id,school_type,type,type_name," \
                  "view_month,view_total,view_total_number,view_week)values(%s,%s,%s,%s,%s," \
                  "%s,%s,%s,%s,%s," \
                  "%s,%s,%s,%s,%s," \
                  "%s,%s,%s,%s,%s," \
                  "%s,%s,%s,%s,%s," \
...
def main():
    start = time.time()
    pool = Pool(5)
    origin_num = [x for x in range(1, 148)]
    try:
